...
// import 'package:data/services/motorhome_habitation_damp_pdf.dart';
...
// class MotorhomeHabitationDampReportForm extends StatefulWidget {
//   const MotorhomeHabitationDampReportForm({super.key});

//   @override
//   State<MotorhomeHabitationDampReportForm> createState() =>
//       _MotorhomeHabitationDampReportFormState();
// }

// class _MotorhomeHabitationDampReportFormState
//     extends State<MotorhomeHabitationDampReportForm> {
//   // Controllers
...
//   List<TextEditingController> sectionCommentControllers =
...
//     'Attach Front Photo', // Step 3 (Motorhome-style)
//     'Attach Rear Photo', // Step 4 (Motorhome-style)
//     'Attach Nearside Photo', // Step 5 (Motorhome-style)
//     'Attach Offside Photo', // Step 6 (Motorhome-style)
...
//       'image': 'assets/images/nearside.png',
//       'key': 'Nearside',
//     },
//     {
//       'title': 'Attach Offside Photo',
//       'image': 'assets/images/offside.png',
//       'key': 'Offside',
//     },
//     {
//       'title': 'Attach Ceiling Photo',
//       'image': 'assets/images/ceiling.png',
//       'key': 'Ceiling',
//     },
//     {
...
//       case 4:
//         return _buildPhotoAttachmentSection(1, isSmallScreen); // Rear
//       case 5:
...
//             children: [
//               Text(
//                 formData.date?.toIso8601String().split('T')[0] ?? 'Enter* Date',
//                 style: TextStyle(
...
//       return _buildViewingFromSummaryButtons(
//           isSmallScreen, currentStep, totalSteps);
//     }

//     return _buildNormalBottomButtons(isSmallScreen, currentStep, totalSteps);
//   }

//   Widget _buildViewingFromSummaryButtons(
//       bool isSmallScreen, int currentStep, int totalSteps) {
//     return Container(
...
//   Widget _buildNormalBottomButtons(
//       bool isSmallScreen, int currentStep, int totalSteps) {
//     return Container(
//       padding: const EdgeInsets.all(20),
//       color: _backgroundColor,
...
//                 _buildSkipToNextButton(isSmallScreen, currentStep),
//             ],
//           ),
//           const SizedBox(height: 12),
//           if (currentStep < totalSteps - 1)
//             _buildProceedNextButtonLarge(isSmallScreen, currentStep)
//           else if (currentStep == totalSteps - 1)
//             _buildViewSummaryButton(isSmallScreen),
//         ],
//       ),
//     );
//   }
...
//   Widget _buildProceedNextButton(
//       bool isSmallScreen, int currentStep, int totalSteps) {
//     return Expanded(
//       child: ElevatedButton(
//         onPressed: () {
...
//       print(
//           'Using uploaded image as logo: ${logoBytes != null && _selectedImage != null}');

//       // Generate PDF with all collected data
//       final pdfBytes =
//           await MotorhomeHabitationDampReportPdfService.generatePdf(
//         formData,
...
//       Uint8List pdfBytes, TourerDampModel formData) async {
//     try {
//       // Use the printing package to share the PDF
//       await Printing.sharePdf(
//         bytes: pdfBytes,
//         filename: 'motorhome-habitation-damp-report.pdf',
//       );
...
//         formData.photoData[sectionKey]!['attachedFile'] =
//             sectionImages[i]!.name;
//         formData.photoData[sectionKey]!['attachedFileBytes'] =
//             sectionImageBytes[i];
//       }
//     }
//     // Add this debug code
//     print('Photo Data Debug:');
//     print(
//         'Front: ${formData.photoData['Front']?['attachedFileBytes'] != null ? "Has image" : "No image"}');
//     print(
//         'Nearside: ${formData.photoData['Nearside']?['attachedFileBytes'] != null ? "Has image" : "No image"}');
//     print(
//         'Offside: ${formData.photoData['Offside']?['attachedFileBytes'] != null ? "Has image" : "No image"}');
...
// Drawing Controller with proper state management
class DrawingController extends GetxController {
  // For each section: drawing points and drawing mode
  final Map<int, List<Offset>> _drawingPoints = {};
  final Map<int, bool> _drawingModes = {};
  final Map<int, Uint8List?> _editedImages = {};

  // Initialize section
  void initializeSection(int sectionIndex) {
    if (!_drawingPoints.containsKey(sectionIndex)) {
      _drawingPoints[sectionIndex] = [];
    }
    if (!_drawingModes.containsKey(sectionIndex)) {
      _drawingModes[sectionIndex] = false;
    }
  }

  // Get drawing points for a section
  List<Offset> getPoints(int sectionIndex) {
    return _drawingPoints[sectionIndex] ?? [];
  }

  // Get drawing mode for a section
  bool getDrawingMode(int sectionIndex) {
    return _drawingModes[sectionIndex] ?? false;
  }

  // Get edited image for a section
  Uint8List? getEditedImage(int sectionIndex) {
    return _editedImages[sectionIndex];
  }

  // Add drawing point
// Update the addPoint method in DrawingController:
void addPoint(int sectionIndex, Offset point) {
  if (!_drawingPoints.containsKey(sectionIndex)) {
    _drawingPoints[sectionIndex] = [];
  }
  ...
  // Toggle drawing mode
  void toggleDrawingMode(int sectionIndex, bool value) {
    _drawingModes[sectionIndex] = value;
    update([sectionIndex]);
  }

  // Save edited image
  void saveEditedImage(int sectionIndex, Uint8List bytes) {
    _editedImages[sectionIndex] = bytes;
    update([sectionIndex]);
  }

  // Clear drawing - COMPLETELY CLEAR EVERYTHING
  void clearDrawing(int sectionIndex) {
    // Clear points
    if (_drawingPoints.containsKey(sectionIndex)) {
      _drawingPoints[sectionIndex]!.clear();
    }
    
    // Clear edited image
    _editedImages.remove(sectionIndex);
  ...
  // Check if section has drawing
  bool hasDrawing(int sectionIndex) {
    final points = _drawingPoints[sectionIndex];
    if (points == null || points.isEmpty) return false;
    
    // Check if there are any non-zero points
    return points.any((point) => point != Offset.zero);
  }
  
  // Clear all data for a section (when removing image)
  void clearAllData(int sectionIndex) {
    _drawingPoints.remove(sectionIndex);
    _drawingModes.remove(sectionIndex);
    _editedImages.remove(sectionIndex);
    update([sectionIndex]);
  }
}

// Simple drawing painter
// Update the SimpleDrawingPainter class to this:

class SimpleDrawingPainter extends CustomPainter {
  final List<Offset> points;

  SimpleDrawingPainter({required this.points});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < points.length - 1; i++) {
      ...
      // Skip zero offsets (break points)
      if (start == Offset.zero || end == Offset.zero) {
        continue;
      }
      
      // Draw the line regardless of bounds for better performance
...
class MotorhomeHabitationDampReportForm extends StatefulWidget {
  ...
  @override
  State<MotorhomeHabitationDampReportForm> createState() => _MotorhomeHabitationDampReportFormState();
}

class _MotorhomeHabitationDampReportFormState extends State<MotorhomeHabitationDampReportForm> {
  // Controllers
  final TourerDampReportStepController stepController = Get.put(TourerDampReportStepController());
  final DrawingController drawingController = Get.put(DrawingController());
  ...
  // Steps 3-8: Photo sections
  List<XFile?> sectionImages = List.generate(6, (_) => null);
  List<Uint8List?> sectionImageBytes = List.generate(6, (_) => null);
  List<Uint8List?> sectionEditedImageBytes = List.generate(6, (_) => null);
  List<TextEditingController> sectionCommentControllers = List.generate(6, (_) => TextEditingController());
  List<String> sectionComments = List.generate(6, (_) => '');
  ...
  // Section data
  final List<Map<String, String>> sectionInstructions = [
    {'title': 'Attach Front Photo', 'image': 'assets/images/front.png', 'key': 'Front'},
    {'title': 'Attach Rear Photo', 'image': 'assets/images/rear.png', 'key': 'Rear'},
    {'title': 'Attach Nearside Photo', 'image': 'assets/images/nearside.png', 'key': 'Nearside'},
    {'title': 'Attach Offside Photo', 'image': 'assets/images/offside.png', 'key': 'Offside'},
    {'title': 'Attach Ceiling Photo', 'image': 'assets/images/ceiling.png', 'key': 'Ceiling'},
    {'title': 'Attach Floor Photo', 'image': 'assets/images/floor.png', 'key': 'Floor'},
  ];
  ...
  void initState() {
    ...
    for (int i = 0; i < 6; i++) {
      drawingController.initializeSection(i);
    }
  }
      ...
      body: SafeArea(
        child: Stack(
          children: [
            ...
            if (_isFullScreen && _fullScreenImageIndex != null)
              _buildFullScreenImage(_fullScreenImageIndex!),
          ],
        ),
      ),
    );
  ...
  Widget _buildStepContent(int step, double width) {
    ...
    switch (step) {
      case 0: return _buildMainInformationSection(isSmallScreen);
      case 1: return _buildCustomerInfoSection(isSmallScreen);
      case 2: return _buildDampReadingsSection(isSmallScreen);
      case 3: return _buildPhotoAttachmentSection(0, isSmallScreen);
      case 4: return _buildPhotoAttachmentSection(1, isSmallScreen);
      case 5: return _buildPhotoAttachmentSection(2, isSmallScreen);
      case 6: return _buildPhotoAttachmentSection(3, isSmallScreen);
      case 7: return _buildPhotoAttachmentSection(4, isSmallScreen);
      case 8: return _buildPhotoAttachmentSection(5, isSmallScreen);
      case 9: return _buildFinalizationSection(isSmallScreen);
    ...
    return _buildFormContainer(
      Column(
        ...
        children: [
                              ...
                              children: [
                                Row(
                                  ...
                                  children: [
                                    ...
                                    const SizedBox(width: 8),
                                    Text("Add* Attachments", style: TextStyle(color: Colors.grey.shade600, fontFamily: _fontFamily)),
                                  ],
                                ),
                                const SizedBox(height: 8),
                                Text("File Type: JPG, PNG, JPEG", style: TextStyle(color: Colors.grey.shade500, fontSize: 11, fontFamily: _fontFamily)),
                              ],
                            ),
                    ),
                  ),
          ...
          const SizedBox(height: 20),
          _buildTextField(workshopNameController, 'Enter* Approved Workshop Name & Address', isSmallScreen, (value) => formData.workshopName = value),
          const SizedBox(height: 20),
          _buildTextField(jobReferenceController, 'Enter* Job Reference/Date', isSmallScreen, (value) => formData.jobReference = value),
        ],
      ),
      isSmallScreen,
    );
    ...
    return _buildFormContainer(
      Column(
        ...
        children: [
          ...
          const SizedBox(height: 25),
          _buildTextField(customerNameController, 'Enter* Customer Name', isSmallScreen, (value) => formData.customerName = value),
          const SizedBox(height: 16),
          _buildTextField(makeModelController, 'Enter* Make & Model', isSmallScreen, (value) => formData.makeModel = value),
          const SizedBox(height: 16),
          _buildTextField(registrationNumberController, 'Enter* Registeration Number', isSmallScreen, (value) => formData.registrationNumber = value),
          const SizedBox(height: 16),
          _buildTextField(crisVinNumberController, 'Enter* CRIS/Vin Number', isSmallScreen, (value) => formData.crisVinNumber = value),
        ],
      ),
      isSmallScreen,
    );
  ...
  Widget _buildDampReadingsSection(bool isSmallScreen) {
    return _buildFormContainer(
      Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('Guidance Notes', style: TextStyle(fontSize: isSmallScreen ? 18 : 20, fontWeight: FontWeight.bold, color: Colors.black, fontFamily: _fontFamily)),
          const SizedBox(height: 20),
          GestureDetector(
            onTap: () => _showGuidanceNoteDialog(context),
            child: Container(
              padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 14),
              decoration: BoxDecoration(color: const Color(0xff173EA6), borderRadius: BorderRadius.circular(4)),
              child: Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  Row(children: [const Icon(Icons.remove_red_eye, color: Colors.white), const SizedBox(width: 8), Text('View* Guidance Note', style: TextStyle(color: Colors.white, fontSize: isSmallScreen ? 14 : 16, fontFamily: _fontFamily))]),
                  const Icon(Icons.open_in_new, color: Colors.white),
                ],
              ),
            ),
          ...
          Container(
            height: 140,
            decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(4), border: Border.all(color: const Color(0xFFD1D5DB), width: 1)),
            child: TextFormField(
              controller: commentsRecommendationsController,
              maxLines: 5,
              onChanged: (value) => formData.commentsRecommendations = value,
              decoration: const InputDecoration(hintText: 'Enter* Comments & Recommendations', hintStyle: TextStyle(color: Colors.grey, fontFamily: _fontFamily), border: InputBorder.none, contentPadding: EdgeInsets.all(16)),
              style: TextStyle(fontSize: isSmallScreen ? 14 : 16, fontFamily: _fontFamily),
            ),
          ),
          const SizedBox(height: 16),
          GestureDetector(
            ...
            child: Container(
              ...
              child: Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  Row(children: [const Icon(Icons.remove_red_eye, color: Colors.white), const SizedBox(width: 8), Text('View* Precaution Note', style: TextStyle(color: Colors.white, fontSize: isSmallScreen ? 14 : 16, fontFamily: _fontFamily))]),
                  const Icon(Icons.open_in_new, color: Colors.white),
                ],
              ),
            ),
  ...
  Widget _buildFinalizationSection(bool isSmallScreen) {
    return _buildFormContainer(
      Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('Finalization', style: TextStyle(fontSize: isSmallScreen ? 20 : 22, fontWeight: FontWeight.bold, color: Colors.black, fontFamily: _fontFamily)),
          const SizedBox(height: 25),
          _buildTextField(dampMeterCalibrationController, 'Enter* Damp Meter Calibration Check Figure', isSmallScreen, (value) => formData.dampMeterCalibration = value),
          const SizedBox(height: 16),
          _buildTextField(weatherConditionsController, 'Enter* Weather Conditions', isSmallScreen, (value) => formData.weatherConditions = value),
          const SizedBox(height: 16),
          _buildTextField(ambientTemperatureController, 'Enter* Ambient Temperature', isSmallScreen, (value) => formData.ambientTemperature = value),
          const SizedBox(height: 16),
          _buildTextField(dampMeterMakeModelController, 'Enter* Damp Meter Make & Model', isSmallScreen, (value) => formData.dampMeterMakeModel = value),
          const SizedBox(height: 16),
          _buildTextField(serviceTechnicianNameController, 'Enter* Service Technician Name', isSmallScreen, (value) => formData.serviceTechnicianName = value),
          const SizedBox(height: 16),
          _buildTextField(technicianSignatureController, 'Enter* Signature', isSmallScreen, (value) => formData.technicianSignatureText = value),
          const SizedBox(height: 16),
          _buildTextField(customerSignatureController, 'Enter* Customer Signature', isSmallScreen, (value) => formData.customerSignatureText = value),
          const SizedBox(height: 16),
          _buildDateField(isSmallScreen),
        ],
      ),
  ...
  Widget _buildTextField(TextEditingController controller, String hintText, bool isSmallScreen, ValueChanged<String> onChanged) {
    return Container(
      height: isSmallScreen ? 50 : 55,
      ...
      child: TextFormField(
        ...
        decoration: InputDecoration(
          ...
          isDense: true,
          hintStyle: TextStyle(color: Colors.grey.shade600, fontSize: isSmallScreen ? 14 : 16, fontFamily: _fontFamily),
        ),
        style: TextStyle(fontSize: isSmallScreen ? 14 : 16, fontFamily: _fontFamily),
        onChanged: onChanged,
      ),
      ...
      child: InkWell(
        ...
        child: Padding(
          ...
          child: Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Text(formData.date?.toIso8601String().split('T')[0] ?? 'Enter* Date', style: TextStyle(color: formData.date != null ? Colors.black : Colors.grey.shade600, fontSize: isSmallScreen ? 14 : 16, fontFamily: _fontFamily)),
              Icon(Icons.calendar_today, color: Colors.grey.shade600, size: 20),
            ],
          ),
        ),
  ...
  Future<void> _selectDate(BuildContext context) async {
    ...
    if (picked != null) setState(() => formData.date = picked);
  }
  ...
  Widget _buildPhotoAttachmentSection(int sectionIndex, bool isSmallScreen) {
    return SingleChildScrollView(
      padding: EdgeInsets.all(isSmallScreen ? 16 : 20),
      child: _buildPhotoSectionContainer(sectionIndex, isSmallScreen),
    );
  }
  ...
  Widget _buildPhotoSectionContainer(int sectionIndex, bool isSmallScreen) {
    ...
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(sectionInfo['title']!, style: TextStyle(fontSize: isSmallScreen ? 23 : 25, fontWeight: FontWeight.bold, color: Colors.black, fontFamily: _fontFamily)),
          const SizedBox(height: 25),
          sectionImageBytes[sectionIndex] != null ? _buildSelectedImageContainer(sectionIndex, isSmallScreen) : _buildPhotoContainer(sectionInfo['image']!, isSmallScreen),
          const SizedBox(height: 30),
          _buildAttachFileSection(sectionIndex),
          const SizedBox(height: 30),
          _buildCommentsField(sectionIndex, isSmallScreen),
  ...
  Widget _buildPhotoContainer(String imagePath, bool isSmallScreen) {
    return Container(
      height: isSmallScreen ? 180 : 220,
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(8), border: Border.all(color: Colors.grey.shade300, width: 1.5)),
      child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        Center(child: Image.asset(imagePath, height: 110, fit: BoxFit.contain, errorBuilder: (context, error, stackTrace) => Image.asset("assets/images/img.png", height: 110, fit: BoxFit.contain))),
      ]),
    );
  }
            ...
            children: [
              Expanded(
                child: GestureDetector(
                  onTap: () => _showImageInFullScreen(sectionIndex),
                  child: Stack(
                    children: [
                      // Image
                      ...
                      Positioned.fill(
                        child: Align(
                          ...
                          child: Container(
                            ...
                            child: Text('Tap to view & draw', style: TextStyle(color: Colors.white, fontSize: 12, fontFamily: _fontFamily)),
                          ),
                        ),
                      ),
                    ],
              ...
              Container(
                ...
                child: Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    if (controller.getEditedImage(sectionIndex) != null)
                      Container(margin: const EdgeInsets.only(right: 8), padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2), decoration: BoxDecoration(color: Colors.blue, borderRadius: BorderRadius.circular(4)), child: const Text('Edited', style: TextStyle(fontSize: 10, color: Colors.white, fontWeight: FontWeight.bold))),
                    IconButton(icon: Icon(Icons.close, size: 20, color: Colors.red.shade700), onPressed: () => _removeImage(sectionIndex)),
                  ],
                ),
              ),
            ],
            ...
            Row(
              children: [
                ...
                RichText(
                  text: TextSpan(
                    children: [
                      TextSpan(text: sectionImages[sectionIndex] != null ? "Image Attached" : "Attach File", style: const TextStyle(color: Colors.white, fontSize: 15, fontWeight: FontWeight.w500, fontFamily: 'PolySans')),
                      if (sectionImages[sectionIndex] == null) const TextSpan(text: " *", style: TextStyle(color: Colors.red, fontSize: 15, fontWeight: FontWeight.w500)),
                    ],
                  ),
                ),
              ],
            ),
            Text("File Type: JPEG, PNG, JPG", style: TextStyle(color: Colors.grey.shade400, fontSize: 12, fontFamily: _fontFamily)),
          ],
        ),
      ),
    );
  ...
  Widget _buildCommentsField(int sectionIndex, bool isSmallScreen) {
    return TextField(
      maxLines: 5,
      controller: sectionCommentControllers[sectionIndex],
      onChanged: (value) => sectionComments[sectionIndex] = value,
      decoration: InputDecoration(
        hintText: "Enter* Comments (Optional)",
        hintStyle: TextStyle(fontFamily: _fontFamily, fontSize: isSmallScreen ? 15 : 17, color: Colors.grey.shade600),
        filled: true,
        fillColor: Colors.white,
        contentPadding: const EdgeInsets.all(16),
        ...
        focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(10), borderSide: BorderSide(color: Colors.grey.shade300, width: 1.8)),
      ),
    );
  }
...
Widget _buildFullScreenImage(int sectionIndex) {
  // Create a global key for the image container
              ...
              if (controller.getDrawingMode(sectionIndex)) {
                ...
                if (renderBox != null) {
                  ...
                  if (localPosition.dx >= 0 && localPosition.dx <= size.width &&
                      localPosition.dy >= 0 && localPosition.dy <= size.height) {
                    controller.addPoint(sectionIndex, localPosition);
                    // Force UI update after each point
                    setStateLocal(() => repaintCounter++);
                  } else {
                    controller.addBreakPoint(sectionIndex);
                    setStateLocal(() => repaintCounter++);
                  }
                }
              }
                ...
                children: [
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        GestureDetector(
                          ...
                          child: Container(
                            decoration: BoxDecoration(
                              color: controller.getDrawingMode(sectionIndex) ? Colors.red : Colors.black54,
                              shape: BoxShape.circle,
                            ),
                            child: Padding(
                              padding: const EdgeInsets.all(12),
                              child: Icon(
                                controller.getDrawingMode(sectionIndex) ? Icons.edit_off : Icons.edit,
                                color: Colors.white,
                                size: 30,
                              ),
                            ),
                    ...
                    Positioned(
                      ...
                      child: Center(
                        child: Container(
                          ...
                          child: Text(
                            'Drawing Mode - Draw on the image',
                            style: TextStyle(
                              color: Colors.white,
                              fontSize: 16,
                              fontFamily: _fontFamily,
                  ...
                  Positioned(
                    ...
                    child: Row(
                      ...
                      children: [
                        // Clear button
                        if (controller.getPoints(sectionIndex).isNotEmpty)
                          GestureDetector(
                            onTap: () {
                              controller.clearDrawing(sectionIndex);
                              setState(() {
                                sectionEditedImageBytes[sectionIndex] = null;
                              });
                              // Force UI update
                              setStateLocal(() => repaintCounter++);
                              ScaffoldMessenger.of(context).showSnackBar(
                                const SnackBar(
                                  content: Text('Drawing cleared'),
                        ...
                        GestureDetector(
                          onTap: () async {
                            try {
                              if (controller.getPoints(sectionIndex).isNotEmpty) {
                                final RenderRepaintBoundary boundary = _imageContainerKey.currentContext!
                                ...
                                // Save to controller
                                controller.saveEditedImage(sectionIndex, pngBytes);
                                
                                // Save to local state for PDF
                                setState(() {
                                  sectionEditedImageBytes[sectionIndex] = pngBytes;
                                });
                                
                                // Exit drawing mode
                                controller.toggleDrawingMode(sectionIndex, false);
                                setStateLocal(() => repaintCounter++);
...
void _showDrawingInstructions() {
  ...
  void _showImageInFullScreen(int sectionIndex) {
    setState(() {
      _fullScreenImageIndex = sectionIndex;
      _isFullScreen = true;
    });
  }
  ...
  void _hideFullScreen() {
    setState(() {
      _fullScreenImageIndex = null;
      _isFullScreen = false;
    });
  }

void _removeImage(int sectionIndex) {
  drawingController.clearAllData(sectionIndex);
  setState(() {
    sectionImages[sectionIndex] = null;
    sectionImageBytes[sectionIndex] = null;
    sectionEditedImageBytes[sectionIndex] = null;
  ...
  Future<void> _pickSingleImage() async {
    try {
      ...
      final XFile? image = await _imagePicker.pickImage(source: ImageSource.gallery, imageQuality: 85, maxWidth: 800, maxHeight: 800);
      if (image != null) {
        setState(() {
          _selectedImage = File(image.path);
          _isUploading = false;
  ...
  Future<void> _pickImageFromGalleryForSection(int sectionIndex) async {
    try {
      final XFile? image = await _imagePicker.pickImage(source: ImageSource.gallery, maxWidth: 800, maxHeight: 800, imageQuality: 70);
      if (image != null) {
        final bytes = await image.readAsBytes();
        drawingController.clearDrawing(sectionIndex);
        setState(() {
          sectionImages[sectionIndex] = image;
          sectionImageBytes[sectionIndex] = bytes;
          sectionEditedImageBytes[sectionIndex] = null;
        });
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Image "${image.name}" selected'), backgroundColor: Colors.green, duration: const Duration(seconds: 2)));
      }
    } catch (e) {
      print('Error picking image: $e');
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Failed to select image'), backgroundColor: Colors.red, duration: Duration(seconds: 3)));
    }
  }
  ...
  Widget _buildSummaryScreen(bool isSmallScreen) {
    return SingleChildScrollView(
      ...
      child: Column(
        children: [
          ...List.generate(stepTitles.length, (index) => Padding(
            padding: const EdgeInsets.only(bottom: 12),
            child: _buildStepTile(index, isSmallScreen),
          )),
          const SizedBox(height: 20),
  ...
  Widget _buildStepTile(int index, bool isSmallScreen) {
    ...
    return Container(
      padding: const EdgeInsets.symmetric(vertical: 14, horizontal: 18),
      decoration: BoxDecoration(color: isFilled ? _primaryColor : Colors.white, borderRadius: BorderRadius.circular(6), border: Border.all(color: isFilled ? Colors.transparent : Colors.black, width: 1.4)),
      child: Row(
        children: [
          Expanded(child: Row(children: [
            Text(stepTitles[index], style: TextStyle(fontSize: isSmallScreen ? 16 : 17, fontWeight: FontWeight.w700, fontFamily: _fontFamily, color: isFilled ? Colors.white : Colors.black)),
            const SizedBox(width: 16),
            Text(isFilled ? "(Filled)" : "(Skipped)", style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600, fontFamily: _fontFamily, color: isFilled ? Colors.white70 : Colors.grey)),
          ])),
          GestureDetector(
            onTap: () {
              stepController.setViewingFromSummary(true);
            ...
            child: Row(children: [
              Text("VIEW", style: TextStyle(fontSize: isSmallScreen ? 15 : 16, fontWeight: FontWeight.w800, fontFamily: _fontFamily, color: isFilled ? Colors.white : Colors.black)),
              const SizedBox(width: 6),
              Icon(Icons.remove_red_eye_outlined, size: 20, color: isFilled ? Colors.white : Colors.black),
            ]),
          ),
        ],
      ),
    ...
    return Column(children: [
      SizedBox(
        ...
        child: OutlinedButton(
          ...
          style: OutlinedButton.styleFrom(side: const BorderSide(color: Colors.black), padding: const EdgeInsets.symmetric(vertical: 14), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(6))),
          child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
            const Icon(Icons.arrow_back, color: Colors.black),
            const SizedBox(width: 8),
            Text("Go Back", style: TextStyle(fontSize: isSmallScreen ? 16 : 18, color: Colors.black, fontWeight: FontWeight.w700, fontFamily: _fontFamily)),
          ]),
        ),
      ),
      const SizedBox(height: 18),
      SizedBox(
        width: double.infinity,
        child: Container(
          decoration: BoxDecoration(gradient: const LinearGradient(colors: [_primaryColor, _secondaryColor], begin: Alignment.topLeft, end: Alignment.bottomRight), borderRadius: BorderRadius.circular(6)),
          child: ElevatedButton(
            onPressed: _exportToPdf,
            style: ElevatedButton.styleFrom(backgroundColor: Colors.transparent, shadowColor: Colors.transparent, padding: const EdgeInsets.symmetric(vertical: 16), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(6))),
            child: Text("Create A PDF", style: TextStyle(fontSize: isSmallScreen ? 16 : 18, fontWeight: FontWeight.w700, fontFamily: _fontFamily, color: Colors.white)),
          ),
        ),
      ),
    ]);
      ...
      return Container(
        ...
        child: Row(children: [
          Expanded(child: OutlinedButton(
            ...
            style: OutlinedButton.styleFrom(foregroundColor: Colors.black, side: const BorderSide(color: Colors.black), padding: EdgeInsets.symmetric(horizontal: isSmallScreen ? 20 : 30, vertical: 12), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8))),
            child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
              const Icon(Icons.arrow_back, size: 16, color: Colors.black),
              const SizedBox(width: 4),
              Text('Go Back', style: TextStyle(fontSize: isSmallScreen ? 14 : 16, color: Colors.black, fontFamily: _fontFamily)),
            ]),
          )),
          const SizedBox(width: 16),
          Expanded(child: ElevatedButton(
            ...
            style: ElevatedButton.styleFrom(backgroundColor: _primaryColor, foregroundColor: Colors.white, padding: EdgeInsets.symmetric(horizontal: isSmallScreen ? 20 : 30, vertical: 12), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8)), elevation: 0),
            child: Text('Proceed Next', style: TextStyle(fontSize: isSmallScreen ? 14 : 16, fontWeight: FontWeight.w600, fontFamily: _fontFamily)),
          )),
        ]),
      );
    }
    ...
    return Container(
      ...
      child: Column(children: [
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          if (currentStep > 0 && currentStep < totalSteps) OutlinedButton(
            onPressed: stepController.previousStep,
            style: OutlinedButton.styleFrom(foregroundColor: Colors.black, side: const BorderSide(color: Colors.black), padding: EdgeInsets.symmetric(horizontal: isSmallScreen ? 30 : 40, vertical: 12), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8))),
            child: Row(children: [
              const Icon(Icons.arrow_back, size: 16, color: Colors.black),
              const SizedBox(width: 4),
              Text('Go Back', style: TextStyle(fontSize: isSmallScreen ? 14 : 16, color: Colors.black, fontFamily: _fontFamily)),
            ]),
          ) else if (currentStep == 0) OutlinedButton(
            onPressed: _showExitConfirmation,
            style: OutlinedButton.styleFrom(foregroundColor: Colors.black, side: const BorderSide(color: Colors.black), padding: EdgeInsets.symmetric(horizontal: isSmallScreen ? 30 : 40, vertical: 12), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8))),
            child: Row(children: [
              const Icon(Icons.arrow_back, size: 16, color: Colors.black),
          ...
          if (currentStep < totalSteps - 1) OutlinedButton(
            onPressed: () {
              stepController.markStepAsSkipped(currentStep);
              stepController.goToStep(currentStep + 1);
            },
            style: OutlinedButton.styleFrom(foregroundColor: _primaryColor, side: const BorderSide(color: _primaryColor), padding: EdgeInsets.symmetric(horizontal: isSmallScreen ? 30 : 40, vertical: 12), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8))),
            child: Text('Skip To Next', style: TextStyle(fontSize: isSmallScreen ? 14 : 16, color: _primaryColor, fontFamily: _fontFamily)),
          ),
        ]),
        const SizedBox(height: 12),
        if (currentStep < totalSteps - 1) SizedBox(
          width: double.infinity,
          child: ElevatedButton(
            onPressed: () {
              stepController.markStepAsFilled(currentStep);
            ...
            style: ElevatedButton.styleFrom(backgroundColor: _primaryColor, foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(horizontal: 24, vertical: 16), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8)), elevation: 0),
            child: Text('Proceed Next', style: TextStyle(fontSize: isSmallScreen ? 16 : 18, fontWeight: FontWeight.w600, fontFamily: _fontFamily)),
          ),
        ) else if (currentStep == totalSteps - 1) SizedBox(
          width: double.infinity,
          child: ElevatedButton(
            onPressed: () {
              stepController.markStepAsFilled(stepTitles.length - 1);
            ...
            style: ElevatedButton.styleFrom(backgroundColor: _primaryColor, foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(horizontal: 24, vertical: 16), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8)), elevation: 0),
            child: Text('View Summary', style: TextStyle(fontSize: isSmallScreen ? 16 : 18, fontWeight: FontWeight.w600, fontFamily: _fontFamily)),
          ),
        ),
      ]),
    );
  ...
  void _showExitConfirmation() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        title: Text('Exit Form?', style: TextStyle(fontFamily: _fontFamily)),
        content: const Text('Are you sure you want to go back? Your progress may not be saved.', style: TextStyle(fontFamily: _fontFamily)),
        actions: [
          TextButton(onPressed: () => Navigator.pop(context), child: Text('Cancel', style: TextStyle(color: Colors.grey.shade600, fontFamily: _fontFamily))),
          TextButton(onPressed: () { Navigator.pop(context); Navigator.pop(context); }, child: Text('Exit', style: TextStyle(color: _primaryColor, fontFamily: _fontFamily))),
        ],
      ),
    );
  }
      ...
      builder: (context) => Dialog(
        ...
        child: Container(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              const Row(children: [Text("• ", style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold)), Text("Guidance Note", style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700, fontFamily: "PolySans"))]),
...
Moisture levels 30% and above – may indicate that structural damage or deterioration is occurring. Possible resealing required to avoid further degrading followed by immediate rectification action, as necessary.
                """, style: TextStyle(fontSize: 15, height: 1.5, color: Colors.grey.shade700, fontFamily: "PolySans"))),
            ),
          ]),
        ),
      ),
    ...
    showDialog(
      ...
      builder: (context) => Dialog(
        ...
        child: Container(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              const Row(children: [Text("• ", style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold)), Text("Precaution Note", style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700, fontFamily: "PolySans"))]),
              GestureDetector(onTap: () => Navigator.pop(context), child: const Icon(Icons.close, size: 26)),
            ]),
            const SizedBox(height: 10),
            Container(height: 1.2, color: Colors.grey.shade300),
...
We would emphasise that the above report accurately reflects the condition of your motorhome at the date stated. These readings may be subject to atmospheric conditions and the company cannot accept any liability for damp, which may become apparent at a future date. Completed in line with the latest AWS Damp Testing Matrix and carried out in accordance with the latest AWS service technician's handbook and manufacturers guidance in relation to areas required to test. Only the inside of the habitation unit is tested. Personal belongings will not be removed from lockers and storage areas, which therefore, may limit the scope of the damp report.
  ...
  Future<void> _exportToPdf() async {
    try {
      ...
      await _transferAllDataToFormData();
      final logoBytes = await _getLogoBytes();
      final pdfBytes = await MotorhomeHabitationDampReportPdfService.generatePdf(formData, logoImageBytes: logoBytes);
      await _saveOrSharePdf(pdfBytes, formData);
      if (Navigator.canPop(context)) Navigator.of(context).pop();
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('PDF generated successfully!'), backgroundColor: Colors.green, duration: Duration(seconds: 3)));
    } catch (e, stackTrace) {
      print('PDF Export Error: $e\n$stackTrace');
      if (Navigator.canPop(context)) Navigator.of(context).pop();
      showDialog(context: context, builder: (context) => AlertDialog(title: const Text('PDF Generation Failed'), content: Text('Error: ${e.toString()}'), actions: [TextButton(onPressed: () => Navigator.pop(context), child: const Text('OK'))]));
    }
  }

  Future<void> _saveOrSharePdf(Uint8List pdfBytes, TourerDampModel formData) async {
    try {
      await Printing.sharePdf(bytes: pdfBytes, filename: 'motorhome-habitation-damp-report.pdf');
      print('PDF shared successfully: ${pdfBytes.length} bytes');
    } catch (e) {
      print('Error sharing PDF: $e');
      throw Exception('Failed to share PDF: $e');
  ...
  Future<Uint8List?> _getLogoBytes() async {
    try {
      if (_selectedImage != null) return await _selectedImage!.readAsBytes();
      final byteData = await rootBundle.load('assets/images/logo.png');
      return byteData.buffer.asUint8List();
    } catch (e) {
      print('Error getting logo bytes: $e');
  ...
  Future<void> _transferAllDataToFormData() async {
    ...
    // Clear and add uploaded images
    formData.uploadedImages.clear();
    if (_selectedImage != null) {
      try {
        formData.uploadedImages.add(await _selectedImage!.readAsBytes());
      } catch (e) {
        print('Error reading image bytes: $e');
      }
    }
    ...
    for (int i = 0; i < sectionInstructions.length; i++) {
      ...
      // Use edited image from controller if available, otherwise use original
      final editedImage = drawingController.getEditedImage(i);
      if (editedImage != null) {
        formData.photoData[sectionKey]!['attachedFile'] = sectionImages[i]?.name ?? 'image_$i';
        formData.photoData[sectionKey]!['attachedFileBytes'] = editedImage;
      } else if (sectionImages[i] != null && sectionImageBytes[i] != null) {
    ...
    print('=== DATA TRANSFER COMPLETE ===');
    for (var entry in formData.photoData.entries) {
      print('${entry.key}: ${entry.value['attachedFileBytes'] != null ? "Has image" : "No image"}');
    }
  }
}